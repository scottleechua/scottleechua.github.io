...
3. On line 5, replace `GHOST_STAFF_TOKEN` with your Ghost staff token, which you can get from clicking your profile in the lower left of the Ghost admin dashboard, then copying `Staff access token`.
...
- **2022-01**: Initial post. This walkthrough owes a debt of gratitude to The Applied Architect's Ghost on Google Cloud tutorial [link now broken] and Brian Burroughs' [Ghost + Caddy tutorial](https://techroads.org/building-a-caddy-container-stack-for-easy-https-with-docker-and-ghost/), which helped me piece together the deployment process.